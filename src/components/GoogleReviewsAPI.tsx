...
interface GoogleReviewsAPIProps {
  maxReviews?: number;
...
const GoogleReviewsAPI: React.FC<GoogleReviewsAPIProps> = ({ maxReviews = 3, showInFooter = false }) => {
  const [placeData, setPlaceData] = useState<GooglePlaceData | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [lastUpdated, setLastUpdated] = useState<Date | null>(null);

  // Fallback data with current numbers
  ...
  useEffect(() => {
    const fetchGoogleReviews = async () => {
      try {
        ...
        const cacheTimeKey = 'google_reviews_cache_time';
        ...
        // In production, this would call your backend API that uses Google Places API
        // Example: const response = await fetch('/api/google-places/reviews');
        
        // For now, simulate API call and use fallback data
        ...
        setPlaceData(fallbackData); // Fallback to current data
        setLoading(false);
      }
    };
    ...
    const interval = setInterval(fetchGoogleReviews, 30 * 60 * 1000);
  ...
  if (loading) {
    return (
      <div className={`${showInFooter ? 'py-4' : 'py-8'} bg-gradient-to-br from-yellow-50 to-orange-50`}>
